...
async function postBankAccount (req, res, next) {
	const { id_banco, agency, account_number } = req.body
	let response
	try {
		response = await postBankAccountService(id_banco, agency, account_number)
	} catch (error) {
		return next(error)
	}
...
async function putBankAccount (req, res, next) {
	const { id, id_banco, agency, account_number } = req.body
	let response
	try {
		response = await putBankAccountService(id, id_banco, agency, account_number)
	} catch (error) {
		return next(error)
	}
