...
async function postBankAccountService (id_banco, agency, account_number) {
	let methodName = 'postBankAccountService'
	let response
	try {
		logInfo(`Entering ${methodName}`, `id_banco = [${id_banco}], agency = [${agency}], account_number = [${account_number}]`, LOG_BANK_ACCOUNT)
		await validateNewBankAccount(id_banco, agency, account_number)
		response = await postBankAccountRepository(id_banco, agency, account_number)
	} catch (error) {
		logError(`Error ${methodName}`, `exception.mensagemLog = [ ${JSON.stringify(error.mensagemLog)} ]`, LOG_BANK_ACCOUNT)
		throw new ErrorHandler(error.mensagem, httpStatus.BAD_REQUEST, false)
	}
...
async function putBankAccountService (id, id_banco, agency, account_number) {
	let methodName = 'putBankAccountService'
	let response
	try {
		logInfo(`Entering ${methodName}`, `id = [${id}], id_banco = [${id_banco}], agency = [${agency}], account_number = [${account_number}]`, LOG_BANK_ACCOUNT)
		await validateUpdateBankAccount(id, id_banco, agency, account_number)
		response = await putBankAccountRepository(id, id_banco, agency, account_number)
	} catch (error) {
		logError(`Error ${methodName}`, `exception.mensagemLog = [ ${JSON.stringify(error.mensagemLog)} ]`, LOG_BANK_ACCOUNT)
		throw new ErrorHandler(error.mensagem, httpStatus.BAD_REQUEST, false)
	}
