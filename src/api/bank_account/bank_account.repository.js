...
async function postBankAccountRepository (id_banco, agency, account_number, transaction = null) {
  let response
  try {
    transaction = await validaTransaction(transaction)
    const QUERY = new PreparedStatement({name: 'insert-new-bank-account', text: INSERT_NEW_BANK_ACCOUNT, values: [id_banco, agency, account_number]})
...
async function putBankAccountRepository (id, id_banco, agency, account_number, transaction = null) {
  let response
  try {
    transaction = await validaTransaction(transaction)
    const QUERY = new PreparedStatement({name: 'update-bank-account', text: UPDATE_BANK_ACCOUNT, values: [id, account_number, agency, id_banco]})
    response = await transaction.query(QUERY)
  } catch (error) {
