...
async function insertNewWaiterRequestRepository (idMesa, idFuncionario, idContaCliente, valorTotal, produtos, transaction = null) {
  let response
  try {
    transaction = await validaTransaction(transaction)
    const QUERY = new PreparedStatement({name: 'insert-new-waiter-request', text: INSERT_NEW_WAITER_REQUEST, values: [idMesa, idFuncionario, idContaCliente, valorTotal, JSON.stringify(produtos)]})
