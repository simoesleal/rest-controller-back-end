...
async function postCustomer (req, res, next) {
	const { name, lastName, birth, sex, status, docs, email, phone, cellphone, preferences, id_endereco } = req.body
	let response
	try {
		response = await postCustomerService(name, lastName, birth, sex, status, docs, email, phone, cellphone, preferences, id_endereco)
	} catch (error) {
		return next(error)
	}
...
async function putCustomer (req, res, next) {
	const { id, name, lastName, birth, sex, status, docs, email, phone, cellphone, preferences, id_endereco } = req.body
	let response
	try {
		response = await putCustomerService(id, name, lastName, birth, sex, status, docs, email, phone, cellphone, preferences, id_endereco)
	} catch (error) {
		return next(error)
	}
